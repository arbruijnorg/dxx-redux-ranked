...
int ai_door_is_openable(object *objp, segment *segp, int sidenum, int currentObjectiveType, int currentObjectiveID, int objectiveInaccessible, int segnum)
{
	...
	if (objp == ConsoleObject) {
		...
		if (ParTime.sideSizes[segnum][sidenum] < ConsoleObject->size * 2) {
			...
			return ((currentObjectiveType == 2 && Walls[currentObjectiveID].type != WALL_OPEN) || currentObjectiveType == 3 || (currentObjectiveType == 1 && (Objects[currentObjectiveID].type == OBJ_CNTRLCEN || (Objects[currentObjectiveID].type == OBJ_ROBOT && Robot_info[Objects[currentObjectiveID].id].boss_flag))));
		}
