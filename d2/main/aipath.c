...
//	}
	...
	while (cur_seg != end_seg) {
		...
		for (sidenum = 0; sidenum < MAX_SIDES_PER_SEGMENT; sidenum++) {
			...
			if (IS_CHILD(segp->children[snum]) && ((WALL_IS_DOORWAY(segp, snum) & WID_FLY_FLAG) || (ai_door_is_openable(objp, segp, snum, currentObjectiveType, currentObjectiveID, cur_seg)))) {
